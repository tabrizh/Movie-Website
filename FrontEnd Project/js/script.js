...
window.onload = () => {
  const trendingMovies = document.querySelector(
    "#trendingMovies .container .row"
  );
  const comingSoonMovies = document.querySelector(
    "#comingSoonMovies .container .row"
  );
  ...
  form.addEventListener("submit", function (e) {
    ...
    if (searchTerm) {
      ...
      document.querySelector(
        ".section-heading"
      ).textContent = `Search results for ${
...
function wishlistAdd(e) {
  let wishListArr = [];
  let cardImgSrc =
    e.target.parentElement.parentElement.firstElementChild.firstElementChild.firstElementChild.getAttribute(
      "src"
    );
  let cardText =
...
function wishlistDelete(e) {
  ...
  Swal.fire({
    ...
    if (result.isConfirmed) {
      ...
      let newArr = wishListArr.filter(function (e) {
        return e !== `${title}`;
      });
      wishListArr = newArr;
      for (const a of document.querySelectorAll("h5")) {
        if (
          ...
          a.parentElement.parentElement.firstElementChild.lastElementChild.classList.toggle(
            "fas"
          );
        }
      }
  ...
  data.forEach((movie) => {
    ...
    movieEl.innerHTML = `
        <div class="card movie">
             <div class="card-image">
                <a href="https://www.google.com/search?q=${title}+movie&oq=hard+mo&aqs=chrome.0.69i59j69i57j0i512l3j0i457i512j0i512l3j0i67.2376j0j9&sourceid=chrome&ie=UTF-8"><img src="${
      IMAGE_URL + poster_path
    }" class="card-img-top movie-image" alt="${title}"></a>
             ...
             <div class="card-body movie-info ps-0 pe-0">
                ...
                <div class="movie-extra-info d-flex justify-content-between">
                    <p class="card-text movie-year">${release_date.slice(
                      0,
                      4
                    )}</p>
